...
public class Client extends ClientEngine {
	...
	public static void frameMode(ScreenMode screenMode) {
		if (frameMode != screenMode) {
			...
			if (screenMode == ScreenMode.FIXED) {
				...
				frameWidth = 766;
				frameHeight = 555;
				WorldController.viewDistance = 15;
				changeChatArea = true;
				changeTabArea = true;
				...
				WorldController.viewDistance = 20;
				changeChatArea = true;
				changeTabArea = true;
			}
	...
	public void drawLoadingText(int percent, String message) {
		...
		// ✅ Use image producer graphics instead of super.graphics
		Graphics g = aRSImageProducer_1109.getImageGraphics();

		if (BACKGROUND != null) {
			g.drawImage(
				BACKGROUND,
				(frameWidth / 2) - (BACKGROUND.getWidth() / 2),
				(frameHeight / 2) - (BACKGROUND.getHeight() / 2),
				null
			);
		}
...
/*TOP*/			{ 8741, 12, 64030, 43162, 7735, 8404, 1701, 38430, 24094, 10153, 56621, 4783, 1341, 16578, 35003, 25239 },
/*BOT*/			{ 25238, 8742, 12, 64030, 43162, 7735, 8404, 1701, 38430, 24094, 10153, 56621, 4783, 1341, 16578, 35003 },
