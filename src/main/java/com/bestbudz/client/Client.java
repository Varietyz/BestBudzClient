...
public class Client extends ClientEngine
{
	...
	private boolean mouseOverTabArea() {
		...
		if (frameWidth >= 1000) {
			...
			return super.mouseY >= y && super.mouseY <= frameHeight;
		} else {
			// Stacked layout (2 rows of 7)
			int yTop = frameHeight - 73;
			int yBottom = frameHeight - 37;
			return (super.mouseY >= yTop && super.mouseY <= yTop + 36)
				|| (super.mouseY >= yBottom && super.mouseY <= frameHeight);
		}
	}
	...
	public void resetImageProducers()
	{
		...
		if (aRSImageProducer_1109 == null || aRSImageProducer_1109.canvasWidth != frameWidth || aRSImageProducer_1109.canvasHeight != frameHeight) {
			aRSImageProducer_1109 = new ImageProducer(FrameConfig.MIN_WIDTH, FrameConfig.MIN_HEIGHT);
		}
	...
	private void drawMinimapResizable()
	{
		if (Configuration.enableStatusOrbs)
		{
			...
			final boolean hoveringXpOrb =
				super.mouseX >= xpOrbX && super.mouseX <= xpOrbX + 26 &&
					super.mouseY >= xpOrbY && super.mouseY <= xpOrbY + 26;
...
// ──────────────── COMPASS ────────────────
		...
		boolean hoveringLogout =
			super.mouseX >= frameWidth - 26 && super.mouseX <= frameWidth - 1 &&
