...
import static com.bestbudz.client.Client.BACKGROUND;
import static com.bestbudz.client.Client.LOGIN_BACKGROUND;
...
public class LoginRenderer {
  ...
  public void displayLoginScreen() {
    ...
    if (client.loginScreenState == 0) {
		Graphics g = Client.aRSImageProducer_1109.getImageGraphics();
		g.drawImage(
			LOGIN_BACKGROUND,
			(Client.frameWidth / 2) - (LOGIN_BACKGROUND.getWidth() / 2),
			(Client.frameHeight / 2) - (LOGIN_BACKGROUND.getHeight() / 2),
			null
		);
      ...
      Client.boldText.method389(
          true, centerX - 326, 0xFFFFFF, "" + ClientConstants.worldSelected, centerY + 228);
      if (client.mouseInRegion(centerX - 373, centerY + 203, centerX - 297, centerY + 240)) {
      ...
      Client.smallText.method389(
          ...
          centerX + 75,
          0xB0AFAB,
          "Client version " + ClientConstants.CLIENT_VERSION,
          centerY + 100);
      Client.smallText.method389(true, centerX - 115, 0xB0AFAB, "Easy login?", centerY + 16);
      Client.smallText.method389(
          true,
          centerX + 40,
          client.mouseInRegion(centerX + 40, centerY + 7, centerX + 127, centerY + 17)
              ? 0xFFFFFF
              : 0xB0AFAB,
          "Too high for this.",
          centerY + 16);
      ...
      if (client.loginMessage1.length() > 0) {
        Client.smallText.method382(
            0xB0AFAB, centerX + 4, client.loginMessage1, centerY + 135, true);
        Client.smallText.method382(
            0xB0AFAB, centerX + 4, client.loginMessage2, centerY + 145, true);
      } else {
        Client.smallText.method382(
            0xB0AFAB, centerX + 4, client.loginMessage2, centerY + 140, true);
      }
    }
  ...
  public void displayAccounts() {
    ...
    if (AccountManager.accounts != null) {
      for (int index = 0; index < 3; index++, y += 47) {
        ...
        if (account.uses != 0
            && client.mouseX >= centerX + 192
            && client.mouseX <= centerX + 212
            && client.mouseY >= y + 21
            && client.mouseY <= y + 40) {
          client.drawTooltip(
              client.mouseX + 11, client.mouseY - 20, "Account uses: " + account.uses);
        }
        Client.newRegularFont.drawCenteredString(
            "Clear Account List",
            centerX + 260,
