...
public class RSFont extends DrawingArea {
  ...
  public static int method1014(
      byte[][] is, byte[][] is_27_, int[] is_28_, int[] is_29_, int[] is_30_, int i, int i_31_) {
    int i_32_ = is_28_[i];
  ...
  public void drawShadowedString(String string, int x, int y, int color, boolean shadow) {
      ...
      if (string.charAt(index) == '@'
        ...
        if (c != ' ') {
          if (shadow) {
            drawCharacter(
                fontPixels[c],
                x + characterDrawXOffsets[c] + 1,
                y + characterDrawYOffsets[c] + 1,
                characterWidths[c],
                characterHeights[c],
                0);
          }
          drawCharacter(
              fontPixels[c],
              x + characterDrawXOffsets[c],
              y + characterDrawYOffsets[c],
              characterWidths[c],
              characterHeights[c],
              color);
        }
    ...
    if (strikeThrough) {
      DrawingArea.drawHorizontalLine(
          lineX,
          y + (int) ((double) baseCharacterHeight * 0.69999999999999996D),
          x - lineX,
          0x800000);
    }
  }

  public void drawStringMoveY(
      String string, int drawX, int drawY, int color, int shadow, int randomMod, int randomMod2) {
    if (string != null) {
  ...
  public void drawCenteredString(String s, int i, int j) {
    if (s != null) drawBasicString(s, i - getTextWidth(s) / 2, j);
  }
  ...
  public void drawCenteredStringMoveXY(
      String string, int drawX, int drawY, int color, int shadow, int randomMod) {
    if (string != null) {
      setColorAndShadow(color, shadow);
  ...
  public void drawCenteredStringMoveY(
      String class100, int drawX, int drawY, int color, int shadow, int i_54_) {
    if (class100 != null) {
      setColorAndShadow(color, shadow);
    ...
    for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
      ...
      if (character == 60 && isSpecial(string, currentCharacter)) {
        ...
        if (startIndex == -1) {
          ...
          if (underlineColor != -1) {
            rsDrawingArea.drawHorizontalLine(
                drawX, drawY + baseCharacterHeight, i_109_, underlineColor);
          }
          drawX += i_109_;
        }
      }
