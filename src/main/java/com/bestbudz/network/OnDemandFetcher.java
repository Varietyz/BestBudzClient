...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
  ...
  private void closeRequest(OnDemandData onDemandData) {
    try {
      ...
      else if (!Client.loggedIn) ioBuffer[3] = 1;
      else ioBuffer[3] = 0;
  ...
  public OnDemandData getNextNode() {
    ...
    try {
      ...
      System.out.println(
          "Failed to unzip model [" + onDemandData.ID + "] type = " + onDemandData.dataType);
      _ex.printStackTrace();
      return null;
    }
  ...
  private void handleFailed() {
    ...
    for (OnDemandData onDemandData = (OnDemandData) requested.reverseGetFirst();
      ...
      if (onDemandData.incomplete) {
        uncompletedCount++;
        System.out.println(
            "Error: model is incomplete or missing  [ type = "
    ...
    while (uncompletedCount < 10) {
      try {
        ...
        waiting = true;
        System.out.println(
            "Error: file is missing  [ type = "
                + onDemandData_1.dataType
                + "]  [id = "
                + onDemandData_1.ID
                + "]");
      } catch (Exception _ex)
  ...
  private void checkReceived() {
    ...
    while (onDemandData != null) {
      ...
      if (clientInstance.decompressors[0] != null)
        abyte0 =
            clientInstance.decompressors[onDemandData.dataType + 1].decompress(onDemandData.ID);
      synchronized (aClass19_1370) {
        if (abyte0 == null) {
