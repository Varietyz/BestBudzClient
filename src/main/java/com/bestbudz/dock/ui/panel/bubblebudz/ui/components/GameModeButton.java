...
public class GameModeButton extends MouseAdapter {
	...
	public void render(Graphics2D g2d, String currentMode) {
		...
		g2d.fill(new RoundRectangle2D.Float(bounds.x, bounds.y, bounds.width, bounds.height, 4, 4));

		Color borderColor = isHovered ? BubbleBudzStyle.TEXT_NEON_CYAN : BubbleBudzStyle.TEXT_SECONDARY;
		g2d.setColor(borderColor);
		g2d.setStroke(new BasicStroke(1f));
		g2d.draw(new RoundRectangle2D.Float(bounds.x, bounds.y, bounds.width, bounds.height, 4, 4));

		g2d.setFont(new Font("SansSerif", Font.PLAIN, 9));
		FontMetrics fm = g2d.getFontMetrics();

		String text = currentMode;
		int textX = bounds.x + (bounds.width - fm.stringWidth(text) - 8) / 2;
		int textY = bounds.y + (bounds.height + fm.getAscent()) / 2 - 1;

		g2d.setColor(isHovered ? BubbleBudzStyle.TEXT_NEON_CYAN : BubbleBudzStyle.TEXT_PRIMARY);
		g2d.drawString(text, textX, textY);

		int arrowX = bounds.x + bounds.width - 8;
		int arrowY = bounds.y + bounds.height / 2;
		drawDownArrow(g2d, arrowX, arrowY, 3);
	}
